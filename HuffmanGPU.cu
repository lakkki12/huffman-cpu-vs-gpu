...
void gpuCompress(const std::string& inputFilePath, const std::string& outputFilePath, GpuTimings& timings) {
    ...
    CUDA_CHECK(cudaMallocHost(&h_outBuf[0], CHUNK_SIZE)); // Output can be up to chunk size in worst case
    ...
    while (currentChunkSize > 0) {
        ...
        // Start reading next chunk from disk in background if there is more data
        if (bytesProcessed + currentChunkSize < originalFileSize) {
            ...
            // Synchronous read here but overlapped with GPU processing since it runs on the CPU
        ...
        histGridSize = std::min(histGridSize, 128); // Cap grid size to avoid too many block atomics
        ...
        // 3. Copy frequencies back to CPU to build the tree
        ...
        CUDA_CHECK(cudaStreamSynchronize(computeStream)); // Must sync here to build the tree on CPU
        ...
        // 7. GPU-only Prefix Scan (Exclusive Scan of Block Sizes)
        ...
        CUDA_CHECK(cudaStreamSynchronize(computeStream)); // Sync to read size
...
void gpuDecompress(const std::string& inputFilePath, const std::string& outputFilePath, GpuTimings& timings) {
    ...
    // Prefetch first chunk read from disk
    auto ioStart = std::chrono::high_resolution_clock::now();
    ...
    while (hasChunk && bytesDecompressed < originalFileSize) {
        ...
        // Rebuild and serialize decoder tables on CPU
        auto cpuTreeStart = std::chrono::high_resolution_clock::now();
        DecEntry h_lut[256];
        ...
        // 2. Launch LUT-Based Parallel Decoding Kernel
        ...
        // 3. Copy Decompressed Chunk back to CPU
        ...
        timings.h2dTime += h2dMs;
        timings.encodeTime += decodeMs; // Re-use encodeTime for decompression kernel time in decompression timings
        timings.d2hTime += d2hMs;
        timings.totalGpuTime += h2dMs + decodeMs + d2hMs;

        // --- Write decompressed output to Disk ---
        ioStart = std::chrono::high_resolution_clock::now();
